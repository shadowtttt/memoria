...
function closeSet(){const el=document.getElementById('settings');el.classList.remove('on');S.stk=[];const tf=document.getElementById('terminal-frame');if(tf)tf.src='about:blank';}
function goSP(pg){if(!document.getElementById('settings').classList.contains('on'))openSet();const cur=S.stk[S.stk.length-1];if(cur){const ce=document.getElementById('sp-'+cur);if(ce){ce.classList.add('out');ce.classList.remove('on');}}S.stk.push(pg);const el=document.getElementById('sp-'+pg);if(el){el.classList.remove('out');el.classList.add('on');}if(pg==='prov')loadProvs();if(pg==='models'){loadProvs();loadMdlSet();}if(pg==='modeltasks')loadModelTasks();if(pg==='mcp')loadMCPs();if(pg==='notes')loadNotes();if(pg==='core')loadCore();if(pg==='conn'){document.getElementById('s-url').value=S.apiUrl;document.getElementById('s-pin').value=S.pin;}if(pg==='data')loadData();if(pg==='memories')loadMemCards(true);if(pg==='usage')loadUsageStats();if(pg==='chatparams')loadChatParams();if(pg==='terminal'){const f=document.getElementById('terminal-frame');if(f&&(!f.src||f.src==='about:blank'))f.src='https://api.catdial.org/terminal/';}}
function backSP(){if(S.stk.length<=1){closeSet();return;}const cur=S.stk.pop();document.getElementById('sp-'+cur).classList.remove('on');if(cur==='terminal'){const tf=document.getElementById('terminal-frame');if(tf)tf.src='about:blank';}const prev=S.stk[S.stk.length-1];document.getElementById('sp-'+prev).classList.remove('out');document.getElementById('sp-'+prev).classList.add('on');}
