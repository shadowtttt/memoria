const S={apiUrl:MEMORIA_MANAGED?MEMORIA_API:'',cid:null,convs:[],allC:[],msgs:[],allMsgs:[],treeMeta:[],hasMoreOlder:false,nextBeforeId:null,hasMoreAfter:false,isPartialView:false,loadingOlder:false,streaming:false,ac:null,models:[],provs:[],stk:[],selModel:localStorage.getItem('memoria_model')||'',attachments:[],curFolder:null,searchQ:'',searchResults:null,searchTimer:null,_hlMsgId:null,recallDebug:localStorage.getItem('memoria_recall_debug')==='1',memoryRecallSettings:null,convDatesCache:{},convCalShown:null,convCalCid:null};
...
document.addEventListener('DOMContentLoaded',()=>{_initTheme();const inp=document.getElementById('msg-input');inp.value='';if(!MEMORIA_MANAGED)return;if(!S.apiUrl){openSet();goSP('conn');}else{Promise.all([loadConvs(),loadMdls()]);}aR(inp);updSub();/* #20 Service Worker registration */if('serviceWorker' in navigator){navigator.serviceWorker.register('sw.js').catch(()=>{});}});
function _initTheme(){const t=localStorage.getItem('memoria_theme')||'light';document.documentElement.setAttribute('data-theme',t);if(t==='dark')document.querySelector('meta[name="theme-color"]').content='#1A1A1A';const sw=document.getElementById('theme-sw');if(sw&&t==='dark')sw.classList.add('on');const rs=document.getElementById('recall-debug-sw');if(rs)rs.classList.toggle('on',S.recallDebug);renderMemoryRecallSwitches();}
...
function renderMemoryRecallSwitches(){const s=S.memoryRecallSettings||{};const ms=document.getElementById('memoria-recall-sw');if(ms)ms.classList.toggle('on',s.memoria_enabled!==false);const ts=document.getElementById('telegram-recall-sw');if(ts)ts.classList.toggle('on',s.telegram_enabled!==false);const ds=document.getElementById('recall-log-sw');if(ds)ds.classList.toggle('on',s.debug_log_enabled!==false);}
async function loadMemoryRecallSettings(){try{const d=await api('get_memory_recall_settings');S.memoryRecallSettings=d?.settings||{};renderMemoryRecallSwitches();}catch(e){}}
async function saveMemoryRecallPatch(patch,label){try{const d=await api('update_memory_recall_settings',{settings:patch});S.memoryRecallSettings=d?.settings||{...(S.memoryRecallSettings||{}),...patch};renderMemoryRecallSwitches();if(label)toast(label);}catch(e){renderMemoryRecallSwitches();toast('保存失败');}}
function toggleMemoriaRecall(){const cur=(S.memoryRecallSettings?.memoria_enabled!==false);saveMemoryRecallPatch({memoria_enabled:!cur},!cur?'Memoria 记忆浮现已开启':'Memoria 记忆浮现已关闭');}
function toggleTelegramRecall(){const cur=(S.memoryRecallSettings?.telegram_enabled!==false);saveMemoryRecallPatch({telegram_enabled:!cur},!cur?'Telegram 记忆浮现已开启':'Telegram 记忆浮现已关闭');}
function toggleRecallLog(){const cur=(S.memoryRecallSettings?.debug_log_enabled!==false);saveMemoryRecallPatch({debug_log_enabled:!cur},!cur?'召回日志已开启':'召回日志已关闭');}
...
function apiErrMsg(e,fallback='失败'){if(e?.name==='AbortError')return'请求超时';if(e?.status===401)return'登录已到期，请重新登录';if(e?.status===400&&String(e.message||'').includes('Unknown action'))return'后端还不支持这个接口';if(e?.status)return fallback+'：HTTP '+e.status;return fallback;}
async function api(a,p={},s=false,externalAc){return memoriaRequest(a,p,s,externalAc);}
...
// Settings nav
function openSet(){document.getElementById('settings').classList.add('on');S.stk=['home'];document.querySelectorAll('.sp').forEach(p=>p.classList.remove('on','out'));document.getElementById('sp-home').classList.add('on');if(S.apiUrl)loadMemoryRecallSettings();}
...
let _termTimer=null;
async function fetchTerminal(){if(document.hidden)return;const el=document.getElementById('terminal-content');if(!el)return;try{const r=await memoriaFetch('/terminal/feed');if(!r.ok)await _throwApiError(r);if(!r.headers.get('content-type')?.includes('text/plain'))throw memoriaSessionError();const t=await r.text();const bd=document.getElementById('terminal-bd');const atBot=bd.scrollHeight-bd.scrollTop-bd.clientHeight<50;el.textContent=t;if(atBot)bd.scrollTop=bd.scrollHeight;}catch(e){el.textContent=apiErrMsg(e,'终端连接失败，请检查网络或重新登录');}}
function _termStart(){if(_termTimer)clearInterval(_termTimer);fetchTerminal();_termTimer=setInterval(fetchTerminal,5000);}
