...
let _termTimer=null;
function fetchTerminal(){fetch('https://api.catdial.org/terminal/',{headers:S.pin?{'x-app-pin':S.pin}:{}}).then(r=>r.text()).then(t=>{const el=document.getElementById('terminal-content');if(!el)return;const bd=document.getElementById('terminal-bd');const atBot=bd.scrollHeight-bd.scrollTop-bd.clientHeight<50;el.textContent=t;if(atBot)bd.scrollTop=bd.scrollHeight;}).catch(()=>{});}
