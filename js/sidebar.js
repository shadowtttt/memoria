...
function saveConn(){return checkMemoriaConnection();}
...
async function loadConv(id){
  /* 切换对话前真正 await 旧流走完（带 1.5s 超时兜底防 UI 卡死） */
  if(S.streaming&&S._activeStream){
    try{stopStr();}catch(e){}
    try{await Promise.race([S._activeStream.donePromise,new Promise(r=>setTimeout(r,1500))]);}catch(e){}
  }
