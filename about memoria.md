...
**背景**：用户不熟悉 Git，且 Claude Code 平台强制每个窗口在独立分支上工作，不能直接 push 到 main。这很容易导致"干了半天 main 一点没动"的问题。

### 分支说明

- **远程默认分支是 `main`**（不是 master）。GitHub Pages 部署在 `main` 上。
- Claude Code 平台只允许 push 到 `claude/` 开头的分支，不能直接 push main。
- 所以每次完工后，需要创建 PR 合并到 main，由用户在 GitHub 网页上点 merge。

### 核心规则

1. **每次完成有意义的工作后，必须创建 PR 合并回 main**。不能只 push 到功能分支就算完了。功能分支上的改动 GitHub Pages 看不到。

2. **开新窗口前，确认上一个窗口的改动已合并到 main**。否则新窗口会基于旧的 main 开始，导致两边分叉、冲突、白干。

3. **完工流程**（每次都要做）：
   ```
   git push -u origin claude/<分支名>
   → 告知用户去 GitHub 网页 merge PR 到 main
   → 用户确认 merge 完成后，才能开新窗口
   ```
...
| # | 坑 | 后果 | 防范 |
|---|-----|------|------|
| 1 | 新窗口基于旧 main 开始 | 和之前的工作分叉，可能白干 | 确认上次 PR 已 merge 再开新窗口 |
| 2 | 两个窗口同时改同一个文件 | merge 时冲突 | 不要同时开两个窗口改前端；如果并行，一个前端一个后端 |
| 3 | Service Worker 缓存旧文件 | 代码更新了但网页没变 | 更新 sw.js 中 CACHE 版本号（如 memoria-v2 → v3）强制刷新；或用户手动：Safari 设置→清除网站数据 |
| 4 | 后端已部署但前端没 merge | 前后端不匹配 | 前后端一起改时，先部署后端，尽快提醒用户 merge 前端 |
| 5 | localStorage / IndexedDB 残留旧数据 | 更新后出现奇怪 bug | 如果更新后行为异常，提醒用户清浏览器缓存 |
| 6 | CLAUDE.md 没合并到 main | 新窗口的 AI 不知道之前做了什么 | 同规则 1，CLAUDE.md 的更新也要及时合并 |
| 7 | PR merge 到了错误的分支 | GitHub Pages 不更新 | PR 的 base 必须是 **main**（不是 master 或其他） |
| 8 | 数据库结构改动 | SQL migration 不在 git 里 | 改表结构时在 Supabase SQL Editor 执行，并把 SQL 记录在 CLAUDE.md 或专门文件中 |

### AI 助手的责任

- **主动提醒**：用户说要"开新窗口"或"换个窗口"时，先检查当前改动是否已通过 PR 合并到 main，没有的话主动提醒。
- **完工检查**：每次任务完成时，主动创建 PR 并告知用户去 merge，不要等用户问。
- **预防问题**：如果发现当前分支和 main 有较大差异，主动告知用户风险。
- **SW 版本号**：每次修改前端文件时，检查是否需要更新 sw.js 的 CACHE 版本号。
